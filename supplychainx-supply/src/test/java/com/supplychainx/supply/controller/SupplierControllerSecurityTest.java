...
import com.supplychainx.security.config.SecurityExpressions;
...
class SupplierControllerSecurityTest {
    ...
    @Import({SecurityExpressions.class, SupplierController.class})
    static class TestConfig {
    }


    @Nested
    @DisplayName("GET /api/suppliers - Récupérer tous les fournisseurs")
    class GetAllSuppliersTests {
        ...
        void testGetAllSuppliers_WithPermission_ShouldSucceed() throws Exception {
            ...
            mockMvc.perform(get("/api/suppliers"))
                ...
                .andExpect(jsonPath("$.data.content[0].name").value("Supplier A"));
        }
    ...
    class DeleteSupplierTests {
        ...
        void testDeleteSupplier_WithPermission_ShouldSucceed() throws Exception {
            mockMvc.perform(delete("/api/suppliers/1").with(csrf()))
    ...
    class AdminRoleTests {
        ...
        @WithMockUser(username = "admin", authorities = {"ROLE_ADMIN", "PERM_SUPPLIER_READ", "PERM_SUPPLIER_CREATE", "PERM_SUPPLIER_UPDATE", "PERM_SUPPLIER_DELETE"})
    ...
    private SupplierRequestDTO createSupplierRequest(String name) {
        return SupplierRequestDTO.builder()
            .code("SUPP-" + name.toUpperCase().replace(" ", "-"))
            .name(name)
            .email("contact@" + name.toLowerCase().replace(" ", "") + ".com")
            .phone("+1234567890")
            .address("123 Main St")
